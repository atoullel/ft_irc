...
std::string SplitCmdPrivmsg(std::string cmd, std::vector<std::string> &tmp)
{
	...
	for (size_t i = 0; i < str1.size(); i++){//split the first string by ',' to get the Salons names
		if (str1[i] == ',')
...
void	Server::CheckForSalons_Clients(std::vector<std::string> &tmp, int fd)
{
	for(size_t i = 0; i < tmp.size(); i++){
		if (tmp[i][0] == '#'){
			...
			if(!GetSalon(tmp[i]))//ERR_NOSUCHNICK (401) // if the Salon doesn't exist
				{senderror(401, "#" + tmp[i], GetClient(fd)->GetFd(), " :No such nick/Salon\r\n"); tmp.erase(tmp.begin() + i); i--;}
			else if (!GetSalon(tmp[i])->GetClientInSalon(GetClient(fd)->GetNickName())) //ERR_CANNOTSENDTOCHAN (404) // if the client is not in the Salon
				{senderror(404, GetClient(fd)->GetNickName(), "#" + tmp[i], GetClient(fd)->GetFd(), " :Cannot send to Salon\r\n"); tmp.erase(tmp.begin() + i); i--;}
			else tmp[i] = "#" + tmp[i];
		}
		else{
...
void	Server::privateMessage(std::string cmd, int fd)
{
	...
	CheckForSalons_Clients(tmp, fd); // check if the Salons and clients exist
	for (size_t i = 0; i < tmp.size(); i++){// send the message to the clients and Salons
		if (tmp[i][0] == '#'){
			tmp[i].erase(tmp[i].begin());
			std::string resp = ":" + GetClient(fd)->GetNickName() + "!~" + GetClient(fd)->GetUserName() + "@localhost PRIVMSG #" + tmp[i] + " :" + message + "\r\n";
			GetSalon(tmp[i])->sendTo_all(resp, fd);
		}
		else{
			std::string resp = ":" + GetClient(fd)->GetNickName() + "!~" + GetClient(fd)->GetUserName() + "@localhost PRIVMSG " + tmp[i] + " :" + message + "\r\n";
